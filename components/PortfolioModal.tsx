...
const PortfolioModal: React.FC<PortfolioModalProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    if (urlParams.get('portfolio-success') === 'true') {
      ...
      if (window.history.replaceState) {
        const cleanUrl = `${window.location.protocol}//${window.location.host}${window.location.pathname}#portfolio`;
  ...
  const nextUrl = typeof window !== 'undefined' ? `${window.location.origin}${window.location.pathname}?portfolio-success=true#portfolio` : '';
           ...
           {showSuccess ? (
             ...
             <>
                ...
                >
                    ...
                    <input type="hidden" name="message" value="Bonjour, je suis intéressé(e) et je souhaiterais voir votre portfolio complet." />
                    <input type="hidden" name="_autoresponse" value="Merci ! Votre demande d'accès à notre portfolio a bien été reçue. Nous vous enverrons le lien par e-mail très prochainement." />
